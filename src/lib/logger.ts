...
export class LoggerImpl implements Logger {
	...
	error(msg: string): void {
		this.outputChannel.error(this.clock.getCurrentTimeText() + ' ' + msg);
		this.outputChannel.show();
	}

	// Displays warn message when log level is set to Warning, Info, Debug, or Trace
    warn(msg: string): void {
        this.outputChannel.warn(this.clock.getCurrentTimeText() + ' ' + msg);
		this.outputChannel.show();
    }

	// Displays log message when log level is set to Info, Debug, or Trace
	log(msg: string): void {
		this.outputChannel.appendLine(this.clock.getCurrentTimeText() + ' ' + msg);
		this.outputChannel.show();
	}

	// Displays debug message when log level is set to Debug or Trace
    debug(msg: string): void {
		this.outputChannel.debug(this.clock.getCurrentTimeText() + ' ' + msg);
		this.outputChannel.show();
	...
	trace(msg: string): void {
		this.outputChannel.trace(this.clock.getCurrentTimeText() + ' ' + msg);
		this.outputChannel.show();
